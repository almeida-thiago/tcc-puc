 ...
 */
export default async (id: string | null = null, person_id: string | null = null, agent_id: string | null = null): Promise<Ticket | Ticket[] > => {
  try {
    if (person_id) {
      const data: Ticket[] = await database.query({
        query: `
          SELECT 
            tickets.*,
