...
/**
 * Adds zero to left
 * @param {number|string} value value do add zero
 * @param {number} digitsLength length to be
 * @returns {string}
 */
export const addZeroLeft = (value: number | string, digitsLength: number) => {
  let number: string = String(value)
  while (number.length < digitsLength) {
    number = `0${number}`
  }
  return number
}
